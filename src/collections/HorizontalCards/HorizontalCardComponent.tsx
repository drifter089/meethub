...
const HorizantalCard: React.FC<Horizontalcard> = ({ heading, content, image }) => {
  return (
    <div className="w-[20rem] md:w-[50rem]  overflow-hidden  bg-foreground duration-300 flex flex-wrap md:flex-nowrap">
      ...
      <div className="md:w-[60%] p-3 flex flex-col justify-center gap-2">
        <h2 className="text-title-bold text-background">{heading}</h2>
        <p className="text-paragraph text-background">{content}</p>
      </div>
    </div>
  )
}
